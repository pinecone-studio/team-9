...
const hasStaticExport = () => fs.existsSync(path.resolve(__dirname, '../../../../../../apps/web/out'));
...
const commentPreview = async ({ webAffected, apiAvailable, webUrl, apiUrl }) => {
  ...
  const body = [
    ...
    webAffected
      ? webUrl !== 'N/A'
        ? `| Web | [${webUrl}](${webUrl}) |`
        : '| Web | Skipped (build did not emit static `apps/web/out`) |'
      : '| Web | Skipped (not affected) |',
...
const main = async () => {
  ...
  if (webAffected) {
    ...
    if (!hasStaticExport()) {
      console.log('Skipping Cloudflare Pages deploy because apps/web/out was not generated by the web build.');
    } else {
