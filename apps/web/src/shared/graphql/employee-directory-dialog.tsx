...
export default function EmployeeDirectoryDialog({ allEmployees, currentUserIdentifier, employee, onClose }: EmployeeDirectoryDialogProps) {
  ...
  const overrideDisabled = !currentUserIdentifier || overridingKey !== null || employeeBenefits.every((benefit) => benefit.status !== "locked");
  ...
  async function handleOverride(benefitId?: string) {
    ...
    try {
      ...
      setActionError(caughtError instanceof Error ? caughtError.message : "Unable to override eligibility.");
    } finally {
    ...
    >
      ...
      >
        <div className="relative flex h-[780px] max-h-[calc(100vh-48px)] w-full max-w-[600px] flex-col overflow-hidden rounded-[12px] border border-[#D4D4D8] bg-white shadow-[0_24px_64px_rgba(15,23,42,0.18)]">
          <button aria-label="Close dialog" className="absolute top-3 right-3 rounded-[8px] p-2 text-[#737373]" onClick={onClose} type="button">
            <X className="h-5 w-5" />
          </button>
          <div className="shrink-0 border-b border-[#E5E5E5] px-6 py-5">
            <div className="flex flex-wrap items-center gap-3 pr-10">
              ...
              <span className={`rounded-[4px] px-2 py-1 text-[12px] leading-4 font-medium ${getStatusBadgeTone(dialogEmployee.employmentStatus)}`}>
                {formatStatusLabel(dialogEmployee.employmentStatus)}
              </span>
            </div>
            <p className="mt-2 text-[14px] leading-5 text-[#737373]">
              {dialogEmployee.position} · {dialogEmployee.department}
            </p>
          </div>

          <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
            {actionError || error ? (
              <div className="mb-4 rounded-[8px] border border-[#FFC9C9] bg-[#FEF2F2] px-4 py-3 text-[14px] leading-5 text-[#B42318]">{actionError ?? error?.message}</div>
            ) : null}
            ...
            {loading && !data ? (
              ...
              <div className="space-y-6">
                <EmployeeDirectoryDialogOverview benefits={employeeBenefits} employee={dialogEmployee} onEditEmployee={() => setIsEditDialogOpen(true)} />
                ...
                />
                <EmployeeDirectoryDialogRecentActions employeeName={dialogEmployee.name} requests={benefitRequests} />
              </div>
            )}
          </div>
        </div>
