...
export default function RuleSectionList({ searchTerm = "" }: RuleSectionListProps) {
  ...
  const [createRuleDefinition] = useCreateRuleDefinitionMutation();
  const [updateRuleDefinition] = useUpdateRuleDefinitionMutation();
  ...
  async function handleAddRule(input: { defaultOperator: Operator; defaultUnit?: string; description: string; name: string; optionsJson?: string; ruleType: RuleType; value: string; valueType: RuleValueType }) {
    if (!activeSection) return;
    setSubmitting(true);
    try {
      const result = await createRuleDefinition({ variables: { input: {
        ...
        description: input.description, isActive: true, name: input.name, optionsJson: input.optionsJson, ruleType: input.ruleType, valueType: input.valueType,
      } } });
      if (!result.data?.createRuleDefinition.id) throw new Error("Failed to create rule definition");
      await refetch();
  ...
  async function handleSaveRule(payload: { description: string; id: string; measurement?: string; name: string; optionsJson?: string; value?: string }) {
    if (!editingRule) return;
    setSubmitting(true);
    try {
      await updateRuleDefinition({ variables: { input: {
